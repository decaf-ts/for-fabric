...
describe("Test Crud Contract", () => {
  beforeAll(async () => {
    // Compile/Transpile the contract to JavaScript
    execSync(
      `npx weaver compile-contract -d --contract-file ./tests/assets/contract/crud-contract/index.ts --output-dir ./docker/infrastructure/chaincode`
    );
    ...
    fs.copyFileSync(
      path.join(
        process.cwd(),
        "./tests/assets/contract/crud-contract/package.json"
      ),
      path.join(process.cwd(), "./docker/infrastructure/chaincode/package.json")
    );
    fs.copyFileSync(
      path.join(
        process.cwd(),
        "./tests/assets/contract/crud-contract/npm-shrinkwrap.json"
      ),
      path.join(
        process.cwd(),
        "./docker/infrastructure/chaincode/npm-shrinkwrap.json"
