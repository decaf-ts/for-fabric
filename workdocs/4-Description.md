...
- Use `@mirror()` to route mirrored reads and writes, with an optional `allow(context)` gate in the 4th argument to bypass all mirror logic when it returns `false`.
